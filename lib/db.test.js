...
test('openDb migrates an existing database file that predates the slots column', () => {
...
test('updateItem rejects slots as a settable field (not admin-editable)', () => {
  const db = openDb(':memory:');
  insertItem(db, { name: 'Foo', dust84: 1, slots: 4 });
  const changed = updateItem(db, 'Foo', { slots: 8 });
  assert.strictEqual(changed, 0);
  assert.strictEqual(getAllItems(db)[0].slots, 4);
  db.close();
});
