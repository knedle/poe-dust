...
test('openDb migrates an old database: adds slots/type/subtype and drops dust83/dust85', () => {
  ...
  `);
  oldDb.prepare('INSERT INTO items (name, dust84, dust83) VALUES (?, ?, ?)').run('Foo', 1, 99);
  oldDb.close();
  ...
  assert.deepStrictEqual(columns.sort(), ['dust84', 'dust84q20', 'name', 'slots', 'subtype', 'type']);
  migrated.close();
...
test('updateItem updates only the given columns and reports affected rows', () => {
  const db = openDb(':memory:');
  insertItem(db, { name: 'Foo', dust84: 1 });
  const changed = updateItem(db, 'Foo', { dust84q20: 5 });
  assert.strictEqual(changed, 1);
  const row = getAllItems(db)[0];
  assert.strictEqual(row.dust84q20, 5);
...
test('updateItem returns 0 for a nonexistent item', () => {
  const db = openDb(':memory:');
  const changed = updateItem(db, 'DoesNotExist', { dust84: 5 });
