...
function openDb(dbPath) {
  ...
  if (!columns.includes('slots')) {
    db.exec('ALTER TABLE items ADD COLUMN slots INTEGER');
  }
  return db;
}
...
function insertItem(db, item) {
  const stmt = db.prepare(`
    INSERT INTO items (name, dust83, dust83q20, dust84, dust84q20, dust85, dust85q20, slots)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
