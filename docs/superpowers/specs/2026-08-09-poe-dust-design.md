...
poe-dust/
  ...
  index.html
  data/
    items.json          # persistent, admin-editable dust data (source of truth)
  cache/
    <league>.json        # ephemeral poe.ninja price cache, 1h TTL (like heist)
  scripts/
    seed.csv              # one-time snapshot of the gist import source
    import-items.js       # one-time script: seed.csv -> data/items.json
  Dockerfile
  docker-compose.yml
  package.json
```
...
(regenerable poe.ninja price snapshots, safe to lose) — this distinction drives
the persistence question deferred below.

## Data model

`data/items.json` is a flat JSON array, the persistent source of truth for dust
values. It does **not** store item type or price — both are derived at request
time by joining against live poe.ninja data by item `name`. An item with no
matching poe.ninja Unique{Weapon,Armour,Accessory} entry in the current league
simply does not appear in the table (no dead rows, no manual type tagging
needed).
...
- `dust<ilvl>q20` — dust yield at that item level with 20% quality. For weapons/armour
  ...
  dust for the quality bonus — the field is reused for that number too (same shape,
  different in-game currency backing it). The UI labels this column so it reads
  correctly for both cases (see UI section).
- All six fields are independently admin-editable. Only `dust84`/`dust84q20` are
...
2. `scripts/import-items.js` — reads `seed.csv`, writes `data/items.json` with:
   - `name` = CSV `name`
...
3. Run once locally against a fresh checkout to produce the initial `data/items.json`,
   which is then committed and becomes the live starting dataset.
...
admin edits in `data/items.json` would be lost if re-run, so this is a
...
- Matching key is exact `name` equality between `data/items.json` and the
...
- `GET /api/items` — returns the full contents of `data/items.json`. Public, no auth.
...
- `PUT /api/admin/items/:name` — requires a valid session cookie (`401` if
  missing/expired). Body is a partial object with any of the six `dust*` fields;
  the matching item in `data/items.json` (by `name`) is updated and the file is
  rewritten to disk. `404` if no item with that name exists.
...
- `Dockerfile`: `FROM node:22-alpine`, copy `server.js`, `index.html`, `data/`,
  `CMD ["node", "server.js"]`.
...
in particular, how `data/items.json` survives redeploys on Render's ephemeral
...
- Production persistence strategy for `data/items.json` on Render.
